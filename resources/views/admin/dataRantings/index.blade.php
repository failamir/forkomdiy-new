        ...
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                ...
                @include('csvImport.modal', [
                    'model' => 'DataRanting',
                    'route' => 'admin.data-rantings.parseCsvImport',
                ])
            </div>
        </div>
    @endcan
                    ...
                    <tbody>
                        @foreach ($dataRantings as $key => $dataRanting)
                            <tr data-entry-id="{{ $dataRanting->id }}">
                                ...
                                <td>
                                    {{ App\Models\Team::where('id', $dataRanting->team_id)->pluck('name')->first() ?? '' }}
                                </td>

                                <td>
                                    @can('data_ranting_show')
                                        <a class="btn btn-xs btn-primary"
                                            href="{{ route('admin.data-rantings.show', $dataRanting->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    ...
                                    @can('data_ranting_edit')
                                        <a class="btn btn-xs btn-info"
                                            href="{{ route('admin.data-rantings.edit', $dataRanting->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('data_ranting_delete')
                                        <form action="{{ route('admin.data-rantings.destroy', $dataRanting->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-DataRanting:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
