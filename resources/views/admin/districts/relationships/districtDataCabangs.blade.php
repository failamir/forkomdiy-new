...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-districtDataCabangs">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $dataCabang->id }}">
                            ...
                            <td>
                                @can('data_cabang_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.data-cabangs.show', $dataCabang->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('data_cabang_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.data-cabangs.edit', $dataCabang->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('data_cabang_delete')
                                    <form action="{{ route('admin.data-cabangs.destroy', $dataCabang->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-districtDataCabangs:not(.ajaxTable)').DataTable({ buttons: dtButtons })
