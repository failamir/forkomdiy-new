...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Village">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($villages as $key => $village)
                        <tr data-entry-id="{{ $village->id }}">
                            ...
                            <td>
                                @can('village_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.villages.show', $village->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('village_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.villages.edit', $village->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('village_delete')
                                    <form action="{{ route('admin.villages.destroy', $village->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-Village:not(.ajaxTable)').DataTable({ buttons: dtButtons })
